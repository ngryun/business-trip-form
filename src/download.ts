...
import { patchSignatureStampBehindTextInHwp } from './hwp-signature-export-patch';
 ...
 */
export async function downloadHwp(wasm: WasmBridge, fileName: string): Promise<void> {
  const bytes = wasm.exportHwp();
  if (!bytes || bytes.length === 0) {
    throw new Error('exportHwp() 가 결과를 반환하지 않았습니다.');
  }
  const patchedBytes = await patchHwpForDownload(bytes);
...
async function patchHwpForDownload(bytes: Uint8Array): Promise<Uint8Array> {
  try {
    return await patchSignatureStampBehindTextInHwp(bytes);
  } catch (err) {
    console.warn('[web-form] HWP 도장 배치 보정 실패:', err);
    return bytes;
  }
}
